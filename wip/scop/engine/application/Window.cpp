...
void
size_callback(GLFWwindow *raw_window, int width, int height)
{
	Window &window = Window::ensure_resolve(raw_window);

	window.fire_size_change(Vector<2, int>(width, height));
}
...
void
Window::fire_size_change(const Vector<2, int> &size)
{
	typedef std::vector<SharedReference<WindowSizeListener> >::iterator iterator_type;

	for (iterator_type iterator = size_listeners.begin(); iterator != size_listeners.end(); ++iterator)
		(*iterator)->on_size_change(*this, size);
